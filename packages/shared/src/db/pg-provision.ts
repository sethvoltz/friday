...
export async function provisionPostgres(opts: {
  ...
  log(`  ZERO_UPSTREAM_DB + ZERO_REPLICA_FILE + ZERO_APP_PUBLICATIONS → ~/.friday/.env`);
  ...
  if (!process.env.ZERO_AUTH_SECRET) {
    throw new Error("ZERO_AUTH_SECRET not set. Call ensureFridayEnv() before provisionPostgres().");
  }
