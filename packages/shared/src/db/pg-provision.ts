...
export async function provisionPostgres(opts: {
  ...
  // Phase 4 (ADR-023): tell zero-cache where to forward custom-mutator
  // server-side runs. The dashboard's `/api/mutators` endpoint hosts
  // the canonical execution via Zero's `PushProcessor`. The URL is
  // localhost-only — the dashboard's reverse-proxy invariant (CFT
  // exposes only the dashboard's public port) keeps it off the public
  // net even though zero-cache itself binds 127.0.0.1.
  upsertEnvVar("ZERO_MUTATE_URL", "http://localhost:5173/api/mutators");
  log(
    `  ZERO_UPSTREAM_DB + ZERO_REPLICA_FILE + ZERO_APP_PUBLICATIONS + ZERO_MUTATE_URL → ~/.friday/.env`,
  );
