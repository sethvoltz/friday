...
describe("syncBigserialSequences", () => {
  test(
    "after raw INSERT with explicit id=150, sequence advances so next nextval > 150",
    async () => {
      const pool = getPool();
      ...
      try {
        ...
        const seqBefore = await client.query(
          "SELECT last_value, is_called FROM mail_id_seq",
        );
  ...
  test(
    "empty table: sequence stays at 1, next nextval returns 1",
    async () => {
