...
vi.mock("../mcp/http.js", () => ({
  daemonFetch: vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({ mails: [] }) }),
}));
...
vi.mock("@friday/shared", async (importOriginal) => {
  ...
  return { ...actual, renderLocalDatetime: vi.fn(() => ""), runHooks: vi.fn().mockResolvedValue([]) };
});
...
describe("assistantMessageHasText", () => {
  ...
  it("is true once a text block is present", async () => {
    const { assistantMessageHasText } = await import("./worker.js");
    expect(assistantMessageHasText({ content: [{ type: "thinking" }, { type: "text", text: "hi" }] })).toBe(true);
  });
  it("is false for a tool_use-only message (deferral is handled separately)", async () => {
    const { assistantMessageHasText } = await import("./worker.js");
    expect(assistantMessageHasText({ content: [{ type: "tool_use", id: "t", name: "Bash" }] })).toBe(false);
  });
...
describe("mid-turn break does not orphan the reply (thinking-first)", () => {
  ...
  async function run(iter: () => AsyncGenerator<Record<string, unknown>>): Promise<Record<string, unknown>[]> {
  ...
  it("captures the reply under THIS turn when the break is armed during thinking", async () => {
    ...
    async function* thinkingThenText(): AsyncGenerator<Record<string, unknown>> {
      ...
      yield { type: "stream_event", session_id: SESSION, event: { type: "message_start", message: { id: "msg_1" } } };
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_start", index: 0, content_block: { type: "thinking" } } };
      // thinking-only assistant boundary — the bug fired the break HERE.
      yield { type: "assistant", session_id: SESSION, message: { id: "msg_1", content: [{ type: "thinking", thinking: "" }] } };
      await tick();
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_stop", index: 0 } };
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_start", index: 1, content_block: { type: "text" } } };
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_delta", index: 1, delta: { type: "text_delta", text: REPLY } } };
      // reply boundary — the break should fire HERE, after the reply streamed.
      yield { type: "assistant", session_id: SESSION, message: { id: "msg_1", content: [{ type: "text", text: REPLY }] } };
      yield { type: "result", session_id: SESSION, stop_reason: "end_turn", usage: { input_tokens: 1, output_tokens: 1, cache_creation_input_tokens: 0, cache_read_input_tokens: 0 }, total_cost_usd: 0 };
    }

    const sent = await run(thinkingThenText);

    const stops = sent.filter((m) => m.type === "block-stop");
    const replyStop = stops.find((m) => typeof m.contentJson === "string" && (m.contentJson as string).includes(REPLY));
  ...
  it("breaks at the first reply boundary for a pure-text turn (no over-deferral)", async () => {
    ...
    async function* pureText(): AsyncGenerator<Record<string, unknown>> {
      yield { type: "system", subtype: "init", session_id: SESSION };
      yield { type: "stream_event", session_id: SESSION, event: { type: "message_start", message: { id: "msg_2" } } };
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_start", index: 0, content_block: { type: "text" } } };
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_delta", index: 0, delta: { type: "text_delta", text: REPLY } } };
      yield { type: "assistant", session_id: SESSION, message: { id: "msg_2", content: [{ type: "text", text: REPLY }] } };
      // If the break (wrongly) over-deferred, the worker would consume this:
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_start", index: 1, content_block: { type: "text" } } };
      yield { type: "stream_event", session_id: SESSION, event: { type: "content_block_delta", index: 1, delta: { type: "text_delta", text: "SHOULD_NOT_APPEAR" } } };
      yield { type: "assistant", session_id: SESSION, message: { id: "msg_2", content: [{ type: "text", text: "SHOULD_NOT_APPEAR" }] } };
      yield { type: "result", session_id: SESSION, stop_reason: "end_turn", usage: { input_tokens: 1, output_tokens: 1, cache_creation_input_tokens: 0, cache_read_input_tokens: 0 }, total_cost_usd: 0 };
    }

    const sent = await run(pureText);

    expect(sent.some((m) => m.type === "block-stop" && typeof m.contentJson === "string" && (m.contentJson as string).includes(REPLY))).toBe(true);
