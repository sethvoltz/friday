...
describe("POST /api/agents/:name/unarchive — contract", () => {
  ...
  it("returns 409 when agent is not archived (working status)", async () => {
    await registry.registerAgent({
      name: "unarchive-working",
      type: "helper",
      parentName: "orchestrator",
    });
    // Transition to working — valid FSM edge from idle.
    await registry.setStatus("unarchive-working", "working");
    expect((await registry.getAgent("unarchive-working"))?.status).toBe("working");

    const res = await post("unarchive-working");
    expect(res.status).toBe(409);
