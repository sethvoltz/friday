...
**Scroll-anchor preservation on prepend.** Naive `scrollTop = beforeTop + (newScrollHeight - oldScrollHeight)` arithmetic is sensitive to layout-flush timing and dies on subpixel rounding. Instead, anchor restore has a single owner in ChatShell: it caches the topmost visible bubble and its `getBoundingClientRect().top` (viewport-relative, since the document is the scroller) on every scroll event, measures how far that bubble moved when the list mutates, and queues one deferred `window.scrollBy` correction. Two triggers feed it: a ResizeObserver on the transcript `.list` (content growth, older-page prepends) and an `onWindowSlide` hook ChatMessages calls after a virtualization slide — load-bearing because a slide mounts and unmounts the same bubble count, so the list's net height can change by ~0px and the RO never fires. Both triggers share one anchor and one pending delta, so whichever runs second measures 0 and double-apply is impossible. The slide/prepend handlers themselves do no restore math (ADR-039). Concrete elements, no scrollHeight math. See ADR-019 §1.

**WebKit / Safari / Orion paint-deferral.** Writing the scroll position while WebKit's scroll thread is still hot (fast-scroll just stopped, momentum still resolving) can defer both the scroll-position commit and the paint of the newly-revealed region until the next user-originated scroll event. The element-scroller fix (ADR-019 §2's synchronous `overflow-y: hidden` toggle) has no home now that the document scrolls — `overflow: hidden` on `<html>`/`<body>` is silently ignored on iOS WebKit (#153852, #240860). Instead, restore writes defer behind a double-`requestAnimationFrame` before the `window.scrollTo` / `window.scrollBy` adjustment, which sidesteps overflow entirely. See ADR-039.
...
- Both Send and Stop carry `onmousedown={(e) => e.preventDefault()}` to prevent iOS from blurring the textarea before `click` fires (without it, the tap snaps the composer down and the click misfires). See `docs/mobile-ux.md` §Mobile Send/Stop for the full rationale.
