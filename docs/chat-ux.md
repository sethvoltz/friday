...
- Below: all other non-archived sessions (active + idle builders, helpers, bare, scheduled-mid-run) with status dots (idle / working / stalled / error, plus a distinct cyan **compacting** state while an agent's context is being compacted — see _Compaction divider_) and unread badges (`agent_message` count).
...
The chat route uses a real **inner scroller** — `.chat-transcript` (`position: absolute; inset: 0; overflow-y: auto`) inside a `position: fixed` visual-viewport-sized column, with the document body hard-locked (`html.chat-scroll-lock`) so nothing scrolls behind it (ADR-041, which supersedes ADR-039's document-scroll model for chat). The composer and pills are overlays within the column; content scrolls under the translucent header/composer via the scroller's padding (the under-glass look). All chat scroll reads/writes go through one settable seam (`doc-scroll.ts`: `setChatScroller` / `scrollToBottom` / `scrollByDelta` / `readScrollY` / `onChatScroll`, with a `window` fallback before the scroller binds), so the same code is element-scroller on chat and never touches the locked document. Never `behavior: "smooth"` on the follow path (WebKit #238497). On navigation into chat an `afterNavigate` hook plus ChatShell's agent-load effect land the transcript at the bottom.
...
**Scroll-anchor preservation on prepend.** Naive `scrollTop = beforeTop + (newScrollHeight - oldScrollHeight)` arithmetic is sensitive to layout-flush timing and dies on subpixel rounding. Instead, anchor restore has a single owner in ChatShell: it caches the topmost visible bubble and its `getBoundingClientRect().top` (measured against the scroller's top edge) on every scroll event, measures how far that bubble moved when the list mutates, and queues one deferred `scrollByDelta` correction (on the inner scroller). Two triggers feed it: a ResizeObserver on the transcript `.list` (content growth, older-page prepends) and an `onWindowSlide` hook ChatMessages calls after a virtualization slide — load-bearing because a slide mounts and unmounts the same bubble count, so the list's net height can change by ~0px and the RO never fires. Both triggers share one anchor and one pending delta, so whichever runs second measures 0 and double-apply is impossible. See ADR-019 §1 / ADR-041.

**WebKit / Safari / Orion paint-deferral.** Writing the scroll position while WebKit's scroll thread is still hot (fast-scroll just stopped, momentum still resolving) can defer both the scroll-position commit and the paint of the newly-revealed region until the next user-originated scroll event. Restore writes defer behind a double-`requestAnimationFrame` (`afterNextPaint`) before the scroll adjustment, so the resized layout commits first. See ADR-019 §2 / ADR-041.
...
- **"Viewing pre-compaction history" pill.** When the user scrolls **above** the most-recent compaction divider, `ChatShell.svelte` shows a sticky overlay pill ("Viewing pre-compaction history") anchored under the chat header (stacked just below the "Loading older messages…" pill, which shares the top band) — same `.floating-pill` affordance. Clicking it scrolls the divider back into view (sliding the virtualization window to mount the divider first). An `IntersectionObserver` on the divider drives the pill's visibility while the divider is mounted; when virtualization has unmounted it, the index of the divider in the full message list is compared against the window: above the window forces the pill on **only** when the user is not pinned to the live tail (so the pill never shows a false positive at the bottom of a long, frequently-compacted session).
- **While compaction is in flight**, a "Compacting context… `M:SS`" indicator (with an elapsed-time readout) shows for the focused agent, and a distinct **cyan sidebar dot** (a slower pulse than the green `working` dot, `--status-compacting`) marks _every_ compacting agent — focused or not. Both are driven by `chat.isAgentCompacting(name)`, which **unions** two signals: the transient `compacting` SSE event (`phase:'start'` → on, `'done'` → off; low-latency) and the durable `agents.compacting_since` column (replicated via Zero; `services/dashboard/src/lib/stores/chat.svelte.ts`). The column is what lets the indicator **reconstruct on reload/reconnect** (while the daemon stays up) — the window where the SSE event alone showed nothing. On a daemon **restart** the boot reconcile (`clearStaleCompacting`) deliberately NULLs the flag (no compaction is in flight at boot); the indicator then re-lights when the resumed turn's compaction re-emits its `start` frame and re-stamps the column — which is the original-gap scenario (a restart-triggered auto-compaction that used to look like a multi-minute freeze), now shown as soon as the post-restart compaction begins and durable across any reload during it. The elapsed-time readout ticks off `compacting_since`, distinguishing "making progress" from "possibly wedged". The daemon (sole status writer) stamps the column on the `compacting` start frame and clears it on done; any transition out of `working` and boot reconcile clear it as backstops, so a worker that dies mid-compaction can't wedge it on. On completion the transient indicator hands off to the settled divider above.
...
`Write` / `Edit` / `MultiEdit` / `NotebookEdit` are **promoted**: their diff renders directly under the tool row, not two clicks deep behind a collapsed `ToolBlock` card. `FileEditRenderer.svelte` is a thin adapter that maps the raw SDK input (snake_case) to `FileDiff`'s camelCase props via the pure, DOM-free `file-edit-input.ts:mapFileEditInput`, computes the aliased filename headline via `synthesizeHeadline` (the same helper the generic `ToolBlock` headline uses), and mounts `FileDiff` directly — it adds **no** `CollapsibleSection` of its own, so the single height cap is the one `FileDiff` already wraps (`collapsedMaxHeight={400}`). Exactly one cap, one scroll container.

- **Contained card + header (FRI-137).** The diff renders inside a contained card matching the chat-block vocabulary (a `border-left` accent rail like `.tool-block` / `.mail-tool-block`). A single header row sits above the body: a file icon, the **aliased filename** (`Editing ~/a/b.ts` / `Writing @apps/foo/index.html`, via `synthesizeHeadline` → `aliasPath`, reading `homeDir` / `dataDir` from `$page.data`), and the **status badge** (`running…` / `done` / `error` / `stopped`, the shared `tool-status.ts:badgeClass` / `statusLabel` mapping that ToolBlock and MailToolBlock also use).
- **The header row IS the disclosure control.** When the diff overflows the cap, the header is a `<button>` carrying `aria-expanded` with the `+` / `−` glyph (`aria-hidden`) at its right edge; clicking it expands/collapses the body. When the diff fits within the cap it renders fully with the header as a non-interactive label (no glyph, no toggle) — there is no longer a standalone "VIEW DIFF" / "VIEW CONTENT" line. The smart toggle lives in `CollapsibleSection` (below), so todo and mail renderers inherit the same "no toggle when it fits" behavior.
