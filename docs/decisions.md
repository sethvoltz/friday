...
- Entry scroll is now router territory: SvelteKit's built-in scroll handling writes a scroll-to-top on navigation that would fight scroll-to-bottom, so the chat route calls `afterNavigate` + `disableScrollHandling()` and then lands at the bottom itself.
