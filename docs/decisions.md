...
## ADR-033 — Curl-install + `friday update`; pre-baked tarballs + fnm-pinned Node; homebrew-friday retired
...
4. **Payload = pre-baked per-platform tarball (FRI-120's "4b").** CI runs `pnpm install --prod` on a `macos-latest` (arm64) runner under the exact pinned Node and bundles `node_modules` INTO the tarball (`packaging/pack.mjs`). On-device `friday update` = download + sha256-verify + extract + symlink-flip + `launchctl kickstart -k`. NO on-device `pnpm install`, NO build. The pre-baked native addons are therefore ABI-matched to the fnm-provisioned runtime. v1 target is darwin-arm64 only; Linux/x64 is a designed-for future matrix row (`uname` dispatch + `friday-<os>-<arch>.tar.gz` asset naming).
...
- **No Linux/x64 build in v1.** The matrix is designed for it (asset naming, `uname` dispatch) but only darwin-arm64 is built.
...
- **No v1.0.0 graduation here.** ADR-033 records the playbook; flipping to 1.0.0 is a later owner-gated commit.

### Bring this ADR back to the table if

- A transitive dep's postinstall writes an absolute pack-time path into a _content_ file (not a shim, not a symlink). The `process.execPath` spawn doesn't cover that — the relocation smoke (extract to a different abs path, boot daemon + dashboard SSR loading `@oxfmt/binding` + a zero-cache spawn that actually serves) is the canary.
